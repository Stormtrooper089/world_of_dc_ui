...
const CustomerPage: React.FC = () => {
  ...
  const departmentTabs = [
    "Complaints",
    "Inspections",
    "Education matters (SSA)",
    "Sports matters",
    "Election",
    "L.A. Branch",
    "Revenue Branch / District Record Room",
    "Forest matters",
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20 gap-4">
            ...
            {/* Desktop Navigation - Scrollable Tabs */}
            <nav className="hidden lg:flex items-center flex-1 min-w-0 mx-4">
              <div
                className="flex items-center space-x-1 overflow-x-auto scrollbar-hide w-full"
                style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
              >
                {departmentTabs.slice(1).map((tab) => (
                  <button
                    key={tab}
                    onClick={() => setActiveTab(tab)}
                    className={`px-3 py-2 rounded-lg font-medium text-xs transition-all duration-200 whitespace-nowrap flex-shrink-0 ${
                      activeTab === tab
                        ? "bg-white text-[#0d47a1] shadow-md"
                        : "text-white hover:bg-white/20 backdrop-blur-md"
                    }`}
            ...
            <div className="flex items-center space-x-4 flex-shrink-0">
              <button
                className={`hidden lg:flex w-10 h-10 rounded-full hover:opacity-90 items-center justify-center transition-all ${
                  isScrolled
                    ? "bg-blue-700"
          ...
          <div
            className={`lg:hidden border-t ${
              isScrolled
                ...
                : "bg-blue-800/95 backdrop-blur-sm border-blue-700/50"
            }`}
          >
            <div className="px-4 py-4 space-y-2">
              {/* Department Tabs in Mobile Menu */}
              <div className="mb-4">
              ...
              <div className="border-t border-blue-700/50 pt-2 space-y-2">
                <button
