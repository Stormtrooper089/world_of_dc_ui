...
const SignUpForm: React.FC = () => {
  ...
  const onRegister = async (data: RegisterData) => {
    try {
      ...
      navigate(`/verify-otp?mobile=${encodeURIComponent(data.mobileNumber)}&flow=register`);
    } catch (err: any) {
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="space-y-4" onSubmit={handleSubmit(onRegister)}>
          <div>
            ...
            <input {...register('mobileNumber')} type="tel" placeholder="9999999999" className="mt-1 block w-full rounded-md border-gray-300" />
          </div>
          <div>
